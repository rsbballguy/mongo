...
public:
    ...
    void recordBegin();
    int recordEnd();
    void awaitNoBgOps(stdx::unique_lock<Mutex>& lk);

    int getOpsInProgCount() const {
        return _opsInProgCount;
    }

    void waitForAnOpRemoval(stdx::unique_lock<Mutex>& lk, OperationContext* opCtx);

private:
    int _opsInProgCount;
    ConditionVariable _noOpsInProg;
    int _opRemovalsCount;
    ConditionVariable _waitForOpRemoval;
};
...
void BgInfo::awaitNoBgOps(stdx::unique_lock<Mutex>& lk) {
    while (_opsInProgCount > 0)
        _noOpsInProg.wait(lk);
}

void BgInfo::waitForAnOpRemoval(stdx::unique_lock<Mutex>& lk, OperationContext* opCtx) {
...
void awaitNoBgOps(stdx::unique_lock<Mutex>& lk, BgInfoMap* bgiMap, StringData key) {
