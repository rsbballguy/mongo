...
public:
     ...
     */
    void setResultAndCancelRemainingWork_inlock(const stdx::lock_guard<Mutex>& lock,
                                                const Result& result);
    void setResultAndCancelRemainingWork_inlock(const stdx::unique_lock<Mutex>& lock,
                                                const Result& result);
...
void CallbackCompletionGuard<Result>::setResultAndCancelRemainingWork_inlock(
    const stdx::lock_guard<Mutex>& lock, const Result& result) {
    _setResultAndCancelRemainingWork_inlock(result);
}

template <typename Result>
void CallbackCompletionGuard<Result>::setResultAndCancelRemainingWork_inlock(
    const stdx::unique_lock<Mutex>& lock, const Result& result) {
