...
void lockAndCall(stdx::unique_lock<Mutex>* lk, const std::function<void()>& fn) {
    if (!lk->owns_lock()) {
        lk->lock();
    }
    fn();
...
public:
     ...
     */
    WaiterGuard(const stdx::unique_lock<Mutex>& lock, WaiterList* list, Waiter* waiter)
...
private:
    const stdx::unique_lock<Mutex>& _lock;
    WaiterList* _list;
    Waiter* _waiter;
};
...
void ReplicationCoordinatorImpl::_reportUpstream_inlock(stdx::unique_lock<Mutex> lock) {
