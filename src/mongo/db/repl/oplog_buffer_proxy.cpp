...
void OplogBufferProxy::shutdown(OperationContext* opCtx) {
    {
        stdx::lock_guard<Mutex> backLock(_lastPushedMutex);
        stdx::lock_guard<Mutex> frontLock(_lastPeekedMutex);
        _lastPushed.reset();
        _lastPeeked.reset();
    }
...
void OplogBufferProxy::push(OperationContext* opCtx,
    ...
    stdx::lock_guard<Mutex> lk(_lastPushedMutex);
    _lastPushed = *(end - 1);
...
bool OplogBufferProxy::waitForData(Seconds waitDuration) {
    {
        stdx::unique_lock<Mutex> lk(_lastPushedMutex);
        if (_lastPushed) {
            return true;
        }
    }
...
bool OplogBufferProxy::peek(OperationContext* opCtx, Value* value) {
    stdx::lock_guard<Mutex> lk(_lastPeekedMutex);
    if (_lastPeeked) {
        *value = *_lastPeeked;
        return true;
    }
