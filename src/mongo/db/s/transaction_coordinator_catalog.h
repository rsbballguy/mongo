...
private:
     ...
     * progress.
     */
    void _waitForStepUpToComplete(stdx::unique_lock<Mutex>& lk, OperationContext* opCtx);
    ...
    ConditionVariable _stepUpCompleteCV;

    // Notified when the last coordinator is removed from the catalog.
    ConditionVariable _noActiveCoordinatorsCV;
};
