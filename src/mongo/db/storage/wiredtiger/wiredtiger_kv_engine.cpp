            ...
            try {
                ...
                if (initialDataTimestamp.asULL() <= 1) {
                    ...
                    if (oplogNeededForRollback.isOK()) {
                        ...
                        // from it.
                        stdx::lock_guard<Mutex> lk(_oplogNeededForCrashRecoveryMutex);
...
StatusWith<std::vector<std::string>> WiredTigerKVEngine::beginNonBlockingBackup(
    ...
    stdx::lock_guard<Mutex> lock(_oplogPinnedByBackupMutex);
...
void WiredTigerKVEngine::setOldestActiveTransactionTimestampCallback(
    ...
    stdx::lock_guard<Mutex> lk(_oldestActiveTransactionTimestampCallbackMutex);
...
Status WiredTigerKVEngine::dropIdent(OperationContext* opCtx, StringData ident) {
    ...
    if (ret == EBUSY) {
        ...
        {
            stdx::lock_guard<Mutex> lk(_identToDropMutex);
...
bool WiredTigerKVEngine::haveDropsQueued() const {
    ...
    stdx::unique_lock<Mutex> lk(_identToDropMutex, stdx::defer_lock);
...
void WiredTigerKVEngine::startOplogManager(OperationContext* opCtx,
    ...
    stdx::lock_guard<Mutex> lock(_oplogManagerMutex);
...
void WiredTigerKVEngine::haltOplogManager() {
    stdx::unique_lock<Mutex> lock(_oplogManagerMutex);
