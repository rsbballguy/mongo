...
long long MobileRecordStore::dataSize(OperationContext* opCtx) const {
    stdx::lock_guard<Mutex> lock(_dataSizeMutex);
...
long long MobileRecordStore::numRecords(OperationContext* opCtx) const {
    stdx::lock_guard<Mutex> lock(_numRecsMutex);
...
public:
    ...
    void rollback() override {
        stdx::lock_guard<Mutex> lock(_rs->_numRecsMutex);
        _rs->_numRecs -= _diff;
    }
...
public:
    ...
    void rollback() override {
        stdx::lock_guard<Mutex> lock(_rs->_dataSizeMutex);
        _rs->_dataSize -= _diff;
    }
