...
public:
    ...
    void pushEvenIfFull(T const& t) {
        stdx::unique_lock<Mutex> lk(_lock);
    ...
    bool empty() const {
        stdx::lock_guard<Mutex> lk(_lock);
        return _queue.empty();
    }
...
private:
     ...
     * Returns when enough space is available.
     */
    void _waitForSpace_inlock(size_t size, stdx::unique_lock<Mutex>& lk) {
    ...
    ConditionVariable _cvNoLongerFull;
