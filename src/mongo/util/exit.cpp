...
Mutex shutdownMutex;
ConditionVariable shutdownTasksComplete;
...
ExitCode waitForShutdown() {
    stdx::unique_lock<Mutex> lk(shutdownMutex);
...
void registerShutdownTask(unique_function<void(const ShutdownTaskArgs&)> task) {
    stdx::lock_guard<Mutex> lock(shutdownMutex);
...
void shutdown(ExitCode code, const ShutdownTaskArgs& shutdownArgs) {
    ...
    {
        stdx::unique_lock<Mutex> lock(shutdownMutex);
