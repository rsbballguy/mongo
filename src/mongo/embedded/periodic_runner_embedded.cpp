...
bool PeriodicRunnerEmbedded::tryPump() {
    stdx::unique_lock<Mutex> lock(_mutex, stdx::try_to_lock);
    if (!lock.owns_lock())
        return false;
    ...
    for (auto it = _Pausedjobs.begin(); it != _Pausedjobs.end();) {
        ...
        {
            stdx::lock_guard<Mutex> jobLock(job._mutex);
...
void PeriodicRunnerEmbedded::PeriodicJobImpl::stop() {
    ...
    stdx::lock_guard<Mutex> masterLock(_periodicRunner->_mutex);
    stdx::lock_guard<Mutex> lk(_mutex);
    if (isAlive(lk)) {
...
void PeriodicRunnerEmbedded::PeriodicJobImpl::setPeriod(Milliseconds ms) {
    stdx::lock_guard<Mutex> masterLk(_periodicRunner->_mutex);
    stdx::lock_guard<Mutex> lk(_mutex);
