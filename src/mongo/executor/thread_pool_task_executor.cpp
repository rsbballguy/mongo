...
public:
    ...
    boost::optional<ConditionVariable> finishedCondition;
    BatonHandle baton;
};
...
public:
    ...
    ConditionVariable isSignaledCondition;
...
ThreadPoolTaskExecutor::~ThreadPoolTaskExecutor() {
    shutdown();
    auto lk = _join(stdx::unique_lock<Mutex>(_mutex));
...
void ThreadPoolTaskExecutor::join() {
    _join(stdx::unique_lock<Mutex>(_mutex));
}

stdx::unique_lock<Mutex> ThreadPoolTaskExecutor::_join(stdx::unique_lock<Mutex> lk) {
    ...
    while (!_unsignaledEvents.empty()) {
        ...
        lk = stdx::unique_lock<Mutex>(_mutex);
    }
    lk.unlock();
    _net->shutdown();
    lk.lock();
