...
void NetworkInterfaceMock::_runReadyNetworkOperations_inlock(stdx::unique_lock<Mutex>* lk) {
...
void NetworkInterfaceMock::_waitForWork_inlock(stdx::unique_lock<Mutex>* lk) {
    if (_waitingToRunMask & kExecutorThread) {
        _waitingToRunMask &= ~kExecutorThread;
        return;
    }
